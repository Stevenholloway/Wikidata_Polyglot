...
var tabURL = "https://id.loc.gov/authorities/names/n84035104.html"
console.log("item-properties variable tabURL: ", tabURL);
var mouseoverURL = "https://www.wikidata.org/wiki/Q34404"
console.log("item-properties 1st variable mouseoverURL: ", mouseoverURL);

//functon to pass mouseover URLs from content script -- unable to create a global variable capable of triggering the Wikidata API query
function contentScriptURL(wikidata) {
	console.log("item-properties script received mouseoverURL message:", wikidata.mouseoverURL);
		var mouseoverURL = wikidata.mouseoverURL

		console.log("item-properties script received 2nd mouseoverURL message:", wikidata.mouseoverURL);

		console.log("2nd mouseoverURL var= : ", mouseoverURL)
		wikidata.mouseoverURL.preventDefault();
}
//demonstrating that the content script URL message isn't scoped properly...
const mouseoverURL2 = mouseoverURL
console.log("3nd mouseoverURL var= : ", mouseoverURL2);
/*
Assign `contentScriptURL()` as a listener to messages from the content script.
*/
chrome.runtime.onMessage.addListener(contentScriptURL);
    ...
    for (var i = 0; i < Object.keys(language_translations).length; i++) {
      if (typeof language_translations[i].lang !== 'undefined') {
        if (language_translations[i].lang.localeCompare(isoLanguage) == 0) {
          var select_entity;
          var select_language;
          select_entity = language_translations[i].select_entity;
          select_language = language_translations[i].select_language;

          $("#boxLabel0").text(select_language);
          $("#boxLabel1").text(select_entity);
          break
        }
      }
    }
...
function specific_website_QID_search(isoLanguage, mouseoverURL) {
  var string = '';
  ...
  var tabURL_stripped = mouseoverURL.replace(/\/$/, "");
...
function general_QID_search(isoLanguage, mouseoverURL) {
  var string = '';
  ...
  $.ajax({
    ...
    success: function (returnedJson) {
      if (returnedJson.results.bindings.length == 0) {
        chrome.browserAction.setIcon({ path: "./EE-black-38.png" });
        //no_result();
		specific_website_QID_search(isoLanguage, mouseoverURL);
      } else {
        for (i = 0; i < returnedJson.results.bindings.length; i++) {
          // concatenate the Q number on the end of the label (Q extracted from iri after 31 other url characters)
          label = returnedJson.results.bindings[i].iriLabel.value + ' (' + returnedJson.results.bindings[i].iri.value.substring(31, returnedJson.results.bindings[i].iri.value.length) + ')'
...
function wiki_QID_search(isoLanguage, mouseoverURL) {
  var encodedTitle = mouseoverURL.substring(1 + mouseoverURL.lastIndexOf("/"));
  var site = mouseoverURL.substring(0, nthIndex(mouseoverURL, "/", 3));
...
function wikidata_QID_search(isoLanguage, mouseoverURL) {
  var encodedTitle = mouseoverURL.substring(1 + mouseoverURL.lastIndexOf("/"));
  var site = mouseoverURL.substring(0, nthIndex(mouseoverURL, "/", 3));
...
function setStatusOptions(isoLanguage) {
  ...
  $("#box1 option:gt(0)").remove();
	if (isoLanguage == 'ar') { $("#box1 option").text("(تحديد)"); }
	if (isoLanguage == 'el') { $("#box1 option").text("(επιλέγω)"); }
	if (isoLanguage == 'en') { $("#box1 option").text("(Select)"); }
	if (isoLanguage == 'da') { $("#box1 option").text("(Vælg)"); }
  if (isoLanguage == 'de') { $("#box1 option").text("(Wählen)"); }
  if (isoLanguage == 'es') { $("#box1 option").text("(Seleccionar)"); }
	if (isoLanguage == 'fr') { $("#box1 option").text("(Sélectionner)"); }
	if (isoLanguage == 'he') { $("#box1 option").text("(בחר)"); }
	if (isoLanguage == 'it') { $("#box1 option").text("(Selezionare)"); }
	if (isoLanguage == 'nl') { $("#box1 option").text("(selecteer)"); }
	if (isoLanguage == 'pt') { $("#box1 option").text("(Selecionar)"); }
	if (isoLanguage == 'ru') { $("#box1 option").text("(Выбрать)"); }
  if (isoLanguage == 'zh-hans') { $("#box1 option").text("(选择)"); }
  if (isoLanguage == 'zh-hant') { $("#box1 option").text("(選擇)"); }

  if (typeof mouseoverURL !== 'undefined') {
    if ((mouseoverURL.includes("commons.wikimedia.org")) ||
      (mouseoverURL.includes("species.wikimedia.org")) ||
      (mouseoverURL.includes(".wikipedia.org")) ||
      (mouseoverURL.includes(".wikibooks.org")) ||
      (mouseoverURL.includes(".wikinews.org")) ||
      (mouseoverURL.includes(".wikiquote.org")) ||
      (mouseoverURL.includes(".wikisource.org")) ||
      (mouseoverURL.includes(".wikiversity.org")) ||
      (mouseoverURL.includes(".wikivoyage.org")) ||
      (mouseoverURL.includes(".wiktionary.org"))
    ) {
      wiki_QID_search(isoLanguage, mouseoverURL)
    } else if (mouseoverURL.includes("wikidata.org")) {
      wikidata_QID_search(isoLanguage, mouseoverURL)
    } else {
      general_QID_search(isoLanguage, mouseoverURL);
    }
  }
}
...
//All tabs API functions suppressed in favor of passing URIs via content scripts
...
//function gettabURL(callback) {
  ...
  //}, function (tabs) {
    ...
    //console.log("Here is the tab URL in item-properties: ", tabURL);
    //callback();
  //});
//}
